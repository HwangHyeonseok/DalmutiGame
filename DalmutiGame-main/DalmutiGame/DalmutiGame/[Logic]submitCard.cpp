...
int sub_fail_check(int card_name, int card_count, char s_name, int s[4][20]) //카드 제출시 오류사항있는지 체크
{
    ...
    if (card_count != beforeCardCount) return 3;    //제출한 카드가 이전 플레이어가 제출한 카드 장수와 같지 않다면 오류코드 3 return
...
int card_sub(char player_name, int playerCards[4][20]) //카드 제출 함수
{
    ...
    {
        for (int j = 0; j < 20; j++) s[i][j] = playerCards[i][j];
    }
...
void check_player_card(int playerCards[4][20]) //실시간 플레이어 카드 상황 출력(print)
{
    for (int i = 0; i < 4; i++) {
